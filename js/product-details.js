(async function(){
  ...
  const cpValid = !!(cp && (cpUnit==='g' || cpUnit==='ml' || cpUnit==='l') && Number(cp.pricePerUnit)>0);
  ...
  if(cpValid){
    ...
    custBtn.type = 'button';
    custBtn.textContent = cp.label || (cpUnit==='g' ? 'تحديد الجرامات' : (cpUnit==='ml' ? 'تحديد المللي' : 'تحديد اللتر'));
    ...
    lbl.textContent = cp.label || (cpUnit==='g' ? 'حدد الجرامات' : (cpUnit==='ml' ? 'حدد المللي' : 'حدد اللتر'));
    ...
    const unitSpan = el('span','pd-custom__unit'); unitSpan.textContent = (cpUnit==='g' ? 'جم' : (cpUnit==='ml' ? 'مل' : 'لتر'));
    row.appendChild(lbl);
    row.appendChild(input);
    row.appendChild(unitSpan);
    customBox.appendChild(row);
    // Help text with price per unit
    const help = el('div','pd-custom__help'); help.textContent = `سعر ${cpUnit==='g'?'الجرام':(cpUnit==='ml'?'المللي':'اللتر')}: ${priceFmt(cp.pricePerUnit)}`;
  ...
  (function renderBadges(){
    ...
    if(product.discount && ['percentage','percent','precent'].includes(String(product.discount.type||'').toLowerCase()) && typeof product.discount.value === 'number') add(`${product.discount.value}%`, 'product-badge product-badge--sale');
    // 3) PreOrder
