(function(){
  ...
  const WHATSAPP_NUMBER = '201129331702'; // بصيغة دولية بدون +
  ...
  const buildLineItems = (state)=>{
    ...
    Object.entries(state||{}).forEach(([id, qtyRaw])=>{
      ...
      const { variantId } = parseId(id);
      const v = getVariant(p, variantId);
      const unitPrice = v ? Number(v.price||0) : Number(p.price||0);
      const origUnitPrice = v ? Number(v.compareAtPrice || v.price || 0) : Number(p.compareAtPrice || p.price || 0);
      items.push({
        id,
        baseId: p.id,
        title: p.title + (v ? ` - ${unitLabel(v.grams)}` : ''),
        variantId: variantId || null,
        variantLabel: v ? unitLabel(v.grams) : null,
        unitPrice,
        origUnitPrice,
        qty,
  ...
  const render = ()=>{
    ...
    ids.forEach((id)=>{
      ...
      const { variantId } = parseId(id);
      const v = getVariant(p, variantId);
      const displayPrice = v ? Number(v.price||0) : Number(p.price||0);
      const li = document.createElement('li');
      li.className = 'cart-item';
      li.innerHTML = `
        <img class="thumb" src="${p.thumbnail || (p.images&&p.images[0]) || 'imgs/honey.jpeg'}" alt="${p.title}">
        <div class="meta">
          <div class="t">${p.title}${v ? ' - ' + unitLabel(v.grams) : ''}</div>
